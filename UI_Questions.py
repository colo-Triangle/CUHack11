...
def confirm_response(confirmation_window, responses):
    ...
    style = ttk.Style(thanks_window)
    style.theme_use('clam')
    style.configure("Blue.TFrame", background="#e6f0ff")
    style.configure("Blue.TLabel", background="#e6f0ff", font=("Arial", 10))
    style.configure("Blue.TButton", font=("Arial", 10))
    
    main_frame = ttk.Frame(thanks_window, padding="20", style="Blue.TFrame")
    main_frame.pack(expand=True, fill="both")
    
    thanks_label = ttk.Label(main_frame, text="Information Confirmed, Thank you.", 
                             style="Blue.TLabel", font=("Arial", 14, "bold"))
    thanks_label.pack(pady=(0, 20))
    
    exit_button = ttk.Button(main_frame, text="Exit", style="Blue.TButton", command=thanks_window.destroy)
